...
## GetCartItems
You can execute the `GetCartItems` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getCartItems(): QueryPromise<GetCartItemsData, undefined>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getCartItems(dc: DataConnect): QueryPromise<GetCartItemsData, undefined>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getCartItemsRef:
```typescript
const name = getCartItemsRef.operationName;
console.log(name);
```
...
### Return Type
Recall that executing the `GetCartItems` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetCartItemsData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetCartItems`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getCartItems } from '@dataconnect/generated';


// Call the `getCartItems()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getCartItems();

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getCartItems(dataConnect);
...
### Using `GetCartItems`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getCartItemsRef } from '@dataconnect/generated';


// Call the `getCartItemsRef()` function to get a reference to the query.
const ref = getCartItemsRef();

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getCartItemsRef(dataConnect);
...
// Variables can be defined inline as well.
const ref = addProductToCartRef({ productId: ..., quantity: ..., });
...
```typescript
placeOrder(): MutationPromise<PlaceOrderData, undefined>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
placeOrder(dc: DataConnect): MutationPromise<PlaceOrderData, undefined>;
...
```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, placeOrder } from '@dataconnect/generated';
...
// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await placeOrder(dataConnect);
...
```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, placeOrderRef } from '@dataconnect/generated';


// Call the `placeOrderRef()` function to get a reference to the mutation.
const ref = placeOrderRef();

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = placeOrderRef(dataConnect);
